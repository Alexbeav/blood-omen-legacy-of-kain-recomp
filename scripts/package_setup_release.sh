...
#
# Writes: dist/Blood-Omen-Legacy-of-Kain-Recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
# Git does not preserve an empty optional launcher_assets/ directory. Only pass
# it to the fail-closed packager when it contains a tracked package asset.
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix Blood-Omen-Legacy-of-Kain-Recomp \
  --exe-name Blood_Omen__Legacy_of_Kain_Recompiled \
  --display-name "Blood Omen: Legacy of Kain Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Blood Omen: Legacy of Kain disc" \
